...
const TARGET_DAY = { year: 2026, month: 4, day: 28 };
  ...
  return [
    {
      ...
      scheduledDate: parseTimeLabel("10:00 AM"),
      images: [ASSETS.breakOutPractice, ASSETS.breakOutCheat],
      caption:
        "Stuck in one pentatonic box? These 2 pages show how to connect positions, slide through the neck, and make your solos sound way more musical. Stop sounding trapped. Comment SAUCE for the link. #guitar #leadguitar"
    },
    {
      ...
      caption:
        "Want the fretboard to stop feeling random? These pages connect scale degrees, triads, arpeggios, and C major mapping so you can see notes instead of guessing. Massive shortcut for cleaner playing. Comment SAUCE for the link. #guitar #musictheory"
    },
    {
      ...
      caption:
        "Your chord progressions don't need more chords. They need better movement. These 2 cheat sheets show voicings and motion you can steal for stronger, darker, more musical progressions tonight. Comment SAUCE for the link. #guitar #songwriting"
    },
    {
      ...
      caption:
        "If your solos fall apart when chords change, start here. These pages connect diatonic triads, 7th arpeggios, and sweep shapes you can actually use in real lines. This is lead guitar glue. Comment SAUCE for the link. #guitar #arpeggios"
    }
  ];
}
...
async function saveComposer(connection) {
  ...
  await waitFor(
    ...
    "composer save",
    functionExpression((modalSelector) => {
      return location.href !== "https://x.com/compose/post" || !document.querySelector(`${modalSelector} [data-testid="tweetButton"]`);
    }, COMPOSE_MODAL),
    30_000,
    400
  );
...
async function waitForScheduledSummary(connection, summary, timeoutMs = 40_000) {
  ...
  while (Date.now() - start <= timeoutMs) {
    ...
    const summaries = await getScheduledSummaries(connection);
...
async function repairExistingPost(connection, post) {
  const summary = xScheduleSummary(post.scheduledDate);
  console.log(`Repairing existing "${post.name}" draft at ${summary}`);
...
async function ensurePost(connection, post) {
  ...
  if (summaries.includes(summary)) {
    ...
    if (state.text === normalizeText(post.caption) && state.mediaCount === post.images.length) {
      console.log(`Verified existing "${post.name}" draft at ${summary}`);
      return;
    }

    await repairExistingPost(connection, post);
    return;
  }
...
async function main() {
  ...
  try {
    ...
    for (const post of posts) {
      await verifyPost(connection, post);
    }
